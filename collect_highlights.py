...
class KarakeepClient:
    ...
    def create_tag(self, name: str) -> Optional[str]:
        ...
        try:
            response = self._make_request("/tags", method="POST", data={"name": name})
    ...
    def create_bookmark(self, text: str, note: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            response = self._make_request("/bookmarks", method="POST", data=data)
            return response.get("id")
        except Exception as e:
    ...
    def add_bookmark_to_list(self, list_id: str, bookmark_id: str) -> bool:
        ...
        try:
            self._make_request(
                f"/lists/{list_id}/bookmarks/{bookmark_id}", method="PUT"
            )
            return True
        except Exception as e:
...
def cmd_publish(args):
    ...
    if args.list_name:
        ...
        if (
            args.list_name.replace("_", "").replace("-", "").isalnum()
            and " " not in args.list_name
        ):
            # Treat as ID
    ...
    for i, item in enumerate(highlights_to_publish, 1):
        ...
        tag_names = [f"book:{book_title}", f"device:{h['device_id']}"]
...
Credentials:
  ...
  or use --email and --password arguments.
    ...
    publish_parser.add_argument(
        "--force", action="store_true", help="Publish even if duplicates are detected"
    )
    publish_parser.add_argument(
        "--list-name",
        type=str,
